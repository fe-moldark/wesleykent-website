...
# Quick note on v2
Before diving into this I will say I have thought of several ways I want to improve this. I have already begun reworking the code but this next version is much more complex - instead of what you are about to read down below, a single password will be hashed in several algorithms and those will be used for different purposes. The first is to generate three numbers that will designate the locations for the pixels with a 4x4 grid (and those three values when combined will match the targeted index value from the PSK), and the other two will act as the first row and column in a large grid. From that row and column it will generate values for the entire grid and use them to shift around individual pixels within a set space, easiest right now has been only 100x100 pixels blocks at a time but I want to increase that. I've written part of this code and it fully removes any trace of any pattern whatsoever, and outside of brute forcing the password to try and identify actual words I don't see how this could be reconstructed. Moving on to the current, working, version...
<br><br><br>
